                ...
                <div class="col-xs-12">
                  <div class="box">
                    <div class="box-header with-border">
                      <h3 class="box-title">Data Skripsi {{ $sub_judul }}</h3>
                    </div><!-- /.box-header -->
                          ...
                          @foreach($makalah as $mk)
                            <tr>
                                ...
                                <div class="btn-group">
                                  ...
                                  <ul class="dropdown-menu" role="menu">
                                    ...
                                    @if($mk->status!="diterima")
                                      <li><a class="btn bg-blue" href="#" onclick="if(confirm('Anda yakin akan mengubah status proposal {{ $mk->id_makalah }} DITERIMA?\nPerubahan ini tidak bisa dikembalikan!')){$('#id_status{{$mk->id_makalah}}').val(2);$('#formid{{$mk->id_makalah}}').submit();}">Diterima</a></li>
                                    @endif
                                    @if($mk->status!="revisi")
                                      <li><a class="btn bg-yellow" href="#" onclick="if(confirm('Anda yakin akan mengubah status proposal {{ $mk->id_makalah }} REVISI?')){$('#id_status{{$mk->id_makalah}}').val(3);$('#formid{{$mk->id_makalah}}').submit();}">Revisi</a></li>
                                    @endif
                                    @if($mk->status!="ditolak")
                                      <li><a class="btn bg-red" href="#" onclick="if(confirm('Anda yakin akan mengubah status proposal {{ $mk->id_makalah }} DITERIMA?\nPerubahan ini tidak bisa dikembalikan!')){$('#id_status{{$mk->id_makalah}}').val(4);$('#formid{{$mk->id_makalah}}').submit();}">Ditolak</a></li>
                                    @endif
                                  </ul>
                                </div>
                              </td>
                              ...
                              <td>
                                ...
                                {!!Form::close()!!}
                                <a title="Download proposal{{ $mk->id_makalah }}.pdf" class="btn btn-xs btn-default" target="_blank" href="{{ url('makalah/'.$mk->id_makalah.'/pdf') }}"><i class="fa fa-file-pdf-o"></i></a>
                                ...
                                <a title="Delete" class="btn btn-xs btn-default" href="#" onclick="if(confirm('Apakah anda yakin akan menghapus proposal {{ $mk->id_makalah }}?')){ $('#formdelete{{$mk->id_makalah}}').submit(); }"><i class="fa fa-close"></i></a>
								@if($mk->status=='diterima')
